...
export async function addOrganization(formData: FormData) {
  ...
  const slug = await getUniqueSlug(supabase, 'organization', firstName, lastName);
  ...
  await sendEmailNotification(
    email, 
    "Complete your CenterKick Organization Registration", 
    `Hello ${firstName}, an admin has registered your agency profile on CenterKick. Use this link to set your password and access your dashboard: ${process.env.NEXT_PUBLIC_SITE_URL}/register?email=${encodeURIComponent(email)}&role=organization`
  );
...
export async function updateOrganization(id: string, data: any) {
  ...
  if (data.first_name || data.last_name) {
    ...
    if (!fname || !lname) {
      ...
      data.slug = await getUniqueSlug(supabase, current?.role || 'organization', fname, lname);
    } else {
...
export async function migrateAllOrganizationSlugs() {
  const supabase = await createClient();
  ...
  for (const profile of (profiles || [])) {
    const newSlug = await getUniqueSlug(supabase, 'organization', profile.first_name, profile.last_name);
...
export async function getAvailableTalent(query: string = '') {
  ...
  let fetchQuery = supabase
    .from('profiles')
    .select('id, slug, first_name, last_name, role, avatar_url, country, status, organization_id')
...
export async function linkTalentToOrganization(organizationUserId: string, profileId: string) {
  const supabase = await createClient();
...
export async function unlinkTalentFromOrganization(profileId: string) {
  const supabase = await createClient();
  ...
  const { error } = await supabase
    ...
    .update({ 
      ...
      organization_status: 'rejected' // Or just null? Schema says pending/accepted/rejected
    })
    .eq('id', profileId);
