...
interface TransferFocusClientProps {
   profiles: Profile[];
   newsPosts: BlogPost[];
}

export default function TransferFocusClient({ profiles, newsPosts }: TransferFocusClientProps) {
   ...
   const filteredProfiles = useMemo(() => {
      const filtered = profiles.filter(profile => {
         // Strictly allow players and coaches only
      ...
      // Filter by role-specific tab logic
      const now = new Date();
      const oneYearFromNow = new Date();
      oneYearFromNow.setFullYear(now.getFullYear() + 1);

      if (activeTab === 'FREE AGENT') {
         // ONLY currently unsigned players
         return filtered.filter(p => (p.role || '').toLowerCase() === 'player' && (!p.current_club || p.current_club.trim() === '' || p.current_club.toLowerCase().includes('free')));
      } else if (activeTab === 'CONTRACT ENDING') {
         // ONLY Players with contract expiring within a year (who are currently signed)
         return filtered.filter(p => {
            if ((p.role || '').toLowerCase() !== 'player' || !p.contract_expiry) return false;
         ...
         return filtered.filter(p => (p.role || '').toLowerCase() === 'coach' && (!p.current_club || p.current_club.trim() === '' || p.current_club.toLowerCase().includes('free')));
      }
         ...
         <div className="bg-[#1a1a1a] py-8 sm:py-14 px-4 border-b-4 border-[#b50a0a]">
            <div className="max-w-[1200px] mx-auto text-white flex flex-col items-center md:items-start text-center md:text-left">
               ...
               <p className="text-xs md:text-sm text-gray-400 mt-2 font-semibold tracking-wide">
                  Monitor verified player contract status and coach availability
               </p>
            </div>
         </div>
                     ...
                     <table className="w-full text-left border-collapse min-w-[900px]">
                        ...
                        <tbody>
                           {paginatedProfiles.map((profile) => {
                              ...
                              const roleSlug = profile.role === 'coach' ? 'coaches' : 'players';
                                    ...
                                    {activeTab === 'FREE AGENT' ? (
                                       <>
                                          ...
                                          <td className="px-6 py-4.5 border-r border-gray-100">
                                             <span className="font-bold text-xs text-[#b50a0a] tracking-wide">{profile.position || 'Player'}</span>
                                          </td>
