...
export function ImpersonationBanner({ targetEmail, targetId, targetRole }: PropsToUse) {
  ...
  return (
    <div className="bg-gradient-to-r from-amber-600 via-amber-700 to-amber-800 text-white px-4 py-2.5 shadow-lg flex items-center justify-between sticky top-0 z-50 text-xs sm:text-sm font-semibold border-b border-amber-500/30 shrink-0">
      ...
      <button
        onClick={handleExit}
        disabled={isPending}
        className="flex items-center gap-1.5 bg-white hover:bg-amber-50 text-amber-950 px-3.5 py-1.5 rounded-xl transition-all font-bold text-xs shadow-md shrink-0 disabled:opacity-50 border border-amber-200/50 hover:scale-105 active:scale-95"
      >
...
type PropsToUse = ImpersonationBannerProps;
